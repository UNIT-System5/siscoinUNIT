    ...
    */
    ...
    if (isset($_SESSION['admin']) || isset($_SESSION['info'])) {
        ...
        */
        ...
        if (isset($_REQUEST['iNewT'])) {
            $nom = $_REQUEST['nomTipo'];

            $typeNewModel = new User();

            $inserNType = $typeNewModel -> insertType($config['infoLogin'], $config['infoPass'], $nom);
        }
        ...
        if (isset($_REQUEST['inNewO'])) {
            $desc = $_REQUEST['descO'];
            $grupo = $_REQUEST['groupO'];
            $dir = $_REQUEST['dirO'];
            $dep = $_REQUEST['depO'];
            $ciudad = $_REQUEST['cityO'];
            $tel = $_REQUEST['telO'];
            $fk_dep = $_REQUEST['depID'];
            $fk_grupo = $_REQUEST['groupID'];

            $inNewOM = new User();
            $inNewO = $inNewOM -> insertNOffice(
                $config['infoLogin'], $config['infoPass'], $desc, $grupo,
                $dir, $dep, $ciudad, $tel, $fk_dep, $fk_grupo
            );
        }
        ...
        if (isset($_REQUEST['listEquipTN'])) {
            $getEqT2M = new User();

            $getEqT2 = $getEqT2M -> getEquipTypeN($config['infoLogin'], $config['infoPass']);

            echo json_encode($getEqT2);
        }

        if (isset($_REQUEST['getBrands'])) {
            $brandM = new User();

            $getBr = $brandM -> getBrands($config['infoLogin'], $config['infoPass']);
        ...
        if (isset($_REQUEST['accanRpF'])) {
            ...
            $upRDM = new User();
            $upRD = $upRDM -> assTEquipment($config['infoLogin'], $config['infoPass'], $idEq, 'Taller', 3);
        }
        ...
        if (isset($_REQUEST['assignEquip'])) {
            ...
            $upEstado = $upEstadoM -> updateEstadoI(
                $config['infoLogin'], $config['infoPass'], $fk_estado, $id, $id_office
            );
        }
        ...
        if (isset($_REQUEST['registerScrap'])) {
            $idTipo = $_REQUEST['typeID'];
            $nomTipo = $_REQUEST['typeNom'];
            $idMarca = $_REQUEST['brandID'];
            $nomMarca = $_REQUEST['brandNom'];

            $scrapM = new User();
            $scrapEq = $scrapM -> scrapEquip(
                $config['infoLogin'], $config['infoPass'], $idTipo, $nomTipo, $idMarca, $nomMarca
            );

            $id = $_REQUEST['id'];

            $updateSM = new User();
            $updateScrap = $updateSM -> assEquipment2($config['infoLogin'], $config['infoPass'], $id);
        }

        if (isset($_REQUEST['assComps'])) {
            $idComp = $_REQUEST['idComp'];
            $idEquip = $_REQUEST['idEquip'];

            $assCompM = new User();
            $assComp = $assCompM -> assComps($config['infoLogin'], $config['infoPass'], $idComp, $idEquip);
        }

        if (isset($_REQUEST['sendWarr'])) {
            $id = $_REQUEST['id'];

            $warrM = new User();
            $warrSend = $warrM -> assTEquipment(
                $config['infoLogin'], $config['infoPass'], $id, 'Garantía', 5
            );
        }

        if (isset($_REQUEST['returnWarr'])) {
            $id = $_REQUEST['id'];

            $warrM = new User();
            $warrSend = $warrM -> assTEquipment(
                $config['infoLogin'], $config['infoPass'], $id, 'Taller', 3
            );
        }

        if (isset($_REQUEST['listComps'])) {
            $compM = new User();
            $compList = $compM -> listComps($config['infoLogin'], $config['infoPass']);
    ...
    if (isset($_SESSION['subB'])) {
        ...
        if (isset($_REQUEST['getReqSubBA'])) {
            $reqModelBA = new User();

            $listReqSUBA = $reqModelBA -> getRequestssubBA($config['subBLogin'], $config['subBPass']);

            echo json_encode($listReqSUBA);
        }
        ...
        if (isset($_REQUEST['listSubBOS'])) {
            $lSBOSM = new User();

            $lSBOS = $lSBOSM -> listSubBOS($config['subBLogin'], $config['subBPass']);

            echo json_encode($lSBOS);
        }
        ...
        if (isset($_REQUEST['lEquipFSB'])) {
            $eqFSBM = new User();
            $eqFSB = $eqFSBM -> listEqFSB($config['subBLogin'], $config['subBPass']);

            echo json_encode($eqFSB);
        }
    }
    ...
    if (isset($_SESSION['oficina'])) {
        ...
        if (isset($_REQUEST['getEquipT'])) {
            ...
            $getT = $typeModel -> getEquipTypeOff($config['ofiLogin'], $config['ofiPass']);
    ...
    if (isset($_SESSION['compras'])) {
        ...
        if (isset($_REQUEST['IE'])) {
            ...
            $newEquip = $IEModel -> IE(
                ...
                $tipo, $fk_tipo, $fk_marca, $fk_prov, 2
            );
    ...
    if (isset($_SESSION['auditoria'])) {
        ...
        if (isset($_REQUEST['lEquipFM'])) {
            ...
            $eqIM = new User();
            $lequipI = $eqIM -> listEqI($config['audLogin'], $config['audPass'], $mail);

            echo json_encode($lequipI);
        }

        if (isset($_REQUEST['listTEquip'])) {
            $equipMod = new User();

            $equipFAU = $equipMod -> listEQAudi($config['audLogin'], $config['audPass']);

            echo json_encode($equipFAU);
        }

        if (isset($_REQUEST['getReqA'])) {
            $reqAudiM = new User();

            $reqAudi = $reqAudiM -> getRequestsAudi($config['audLogin'], $config['audPass']);

            echo json_encode($reqAudi);
        }
    }
