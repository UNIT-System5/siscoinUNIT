<?php 
    ...
    class User {
        ...
        public function assComps($user, $password, $idComp, $idEquip) {
            $assC = "INSERT INTO asign_comp (fecha_asign, fk_comp, fk_equip)
            VALUES (NOW(), '$idComp', '$idEquip');
        ...
        public function listComps($user, $password) {
            $lComps = "SELECT * FROM equipamiento
            WHERE estado_equip = 'Stock'
            AND desc_equip NOT IN ('Computadora', 'Impresora')
            GROUP BY desc_equip;";

            $queryComps = DBUserConnect::sqlCU($user, $password) -> query($lComps);

            while ($rComps = $queryComps -> fetch_assoc()) {
                $this -> comps[] = $rComps;
            }
