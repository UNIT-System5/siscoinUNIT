...
function lEquip(e = true) {
    ...
    call.done((data) => {
        ...
        data.forEach((d) => {
            ...
            lTr += `<tr>
                <td>${d.id_equip}</td>
                <td><span title="${d.desc_equip}">${d.desc_equip}</span></td>
                <td>${d.marca_equip}</td>
                ...
                class="fas fa-toolbox"></i><i title="Assign Components" id="${d.id_equip}" class="fas fa-wrench"></i>` : ''}
...
function assignComp(id, nomEquip) {
    ...
    $.post('../../init.php', {
        ...
        data.forEach((d) => {
            compOpt += `<option value="${d.id_equip}">${d.desc_equip}</option>`;
        });

        let assignCContainer = `<div class="compContainer">
            <form action="../../init.php" method="POST" id="assCompForm" autocomplete="off">
                <i class="fas fa-times"></i>
                <div class="formTitle">
                ...
                <div class="compSub">
                    <button class="addComp" value="${id}">Add <i class="fas fa-plus"></i></button>
                    <button class="subcomp" value="${id}"><i class="fas fa-paper-plane"></i></button>
                </div>
            </form>
        </div>`;

        $('.container1').prepend(assignCContainer);
        $('.compContainer').fadeTo(400, 1);
    }, 'json');
}
...
$('body').on('click', '.actions .fa-wrench', function() {
    let id = $(this).attr('id');
    ...
    $.post('../../init.php', {
        ...
        assignComp(id, nomEquip);
    }, 'json');
});

let compObj = [];

$('body').on('submit', '#assCompForm', function(e) {
    e.preventDefault();

    let idEquip = $('.subcomp').val();
    let btnPress = $(document.activeElement).attr('class');
    let compID = $('.sComponent').val();
    ...
    if (btnPress == 'addComp') {
        compObj.push(compData);

        $('.sComponent').val('');

        if (!($('.retComp').length)) {
            $('.addComp').before(`<button type="button" class="retComp">
                <i title="return" class="fas fa-chevron-left"></i>
            </button>`);

            $('.retComp').fadeTo(400, 1);
        }
    } else if (btnPress == 'subcomp') {
        compObj.push(compData);
        ...
        compObj.forEach((d) => {
            $.post('../../init.php', {
                ...
                $('.compContainer').toggleClass('active');
                setTimeout(() => {
                    $('.compContainer').fadeTo(400, 0, () => {
...
$('body').on('click', '.retComp', () => {
    let compS = compObj.pop();

    $('.sComponent').val(compS.compID);

    if (compObj.length == 0) {
        $('.retComp').css('pointer-events', 'none');
        $('.retComp').fadeTo(400, 0, () => {
            $('.retComp').remove();
        });
    }
});

$('body').on('click', '#assCompForm .fa-times', () => {
    compObj = [];
