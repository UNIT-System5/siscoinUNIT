    ...
    gGroups().done((data) => {
        ...
        $.post('../../init.php', {
            ...
            let inputMod = `<td>${data[0].id_user}</td>
                ...
                <td>
                    <div class="selectMod">
                        <select name="mOfic" class="modO" form="formUser" disabled required>
                            <option value="" class="default1" disabled selected>Select an Office</option>
                        </select>
                    </div>
                </td>
                <td>
            ...
            $(`#${idM} td:not(:first-child)`).fadeTo(100, 0, () => {
                ...
                $(`body #${idM} input`).add(`#${idM} .selectMod`).
                add(`#${idM} .fa-check`).add(`#${idM} .fa-times`).fadeTo(100, 1);
    
                $('.modG').val(data[0].fk_grupo);

                cOffices(idM).done((data) => {
                    data.forEach((d) => {
                        lOffices += `<option value="${d.id_lugar}">${d.desc_lugar}</option>`
                    });
        
                    $('.modO .default1').nextAll().remove();
                    $('.modO .default1').after(lOffices);
                    $('.modO').removeAttr('disabled');
                    
                    if ($(`.modO option[value="${officeMod}"]`).length) {
                        $('.modO').val(officeMod);
                    } else {
                        $('.modO').val('');
                    }
...
$(document.body).on('change', '.modG', function(e) {
    let lOffices = '';

    if ($('.modG option:selected')) {
        let id = $('.modG').val();
            ...
            <form action="../../init.php" method="POST" id="formCreate" autocomplete="off">
                ...
                <div class="selectOffice">
                    <select name="cOffice" id="cOffice" disabled required>
                        <option class="default" value="" disabled selected>Select an Office</option>
                    </select>
                </div>
...
$('body').on('submit', '#formCreate', (e) => {
    ...
    if (!($.isEmptyObject(office))) {
        ...
        $.post('../../init.php', {
            ...
            $('#cName').val('');
            $('#cOffice').val('');
            $('#cOffice').prop('disabled', 'disabled');
            $('#cGroup').val('');
        });
    }
});