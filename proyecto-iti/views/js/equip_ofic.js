...
function lEquip() {
    ...
    $.post('../../init.php', {
        ...
        data.forEach((d) => {
            if (d.estado_equip == 'Taller' || d.falla == 1) {
                failEq = '';
            } else if (d.estado_equip == 'Instalado' && d.falla != 1) {
                failEq = `<i aria-hidden="true" title="Equip is failing?"
                class="fas fa-wrench repFailI" id="${d.id_equip}"></i>`
            }
